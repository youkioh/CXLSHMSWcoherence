...
int swmc__perf_event_init(struct perf_event *event, unsigned long nr_pages)
{
    struct perf_buffer *rb = NULL;
    int ret = 0, flags = 0;
...
unlock:
    ...
    mutex_unlock(&event->mmap_mutex);
    return ret;
}EXPORT_SYMBOL_GPL(swmc__perf_event_init);

int swmc__perf_event_open (struct perf_event_attr *attr_ptr, pid_t pid,
...
err_fd:
	put_unused_fd(event_fd);
	return err;
}EXPORT_SYMBOL_GPL(swmc__perf_event_open);
