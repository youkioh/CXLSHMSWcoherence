...
static int swmc_kmsg_handle_fetch(struct swmc_kmsg_message *msg)
{
    ...
    if (!msg || msg->header.type != SWMC_KMSG_TYPE_FETCH) {
        pr_err("[%s] Invalid fetch message\n", __func__);
        return -EINVAL;
    }

    pr_info("[%s] Handling fetch message for offset 0x%lx\n", __func__, payload->cxl_hdm_offset);
    ...
    if (!page_replica) { // I -> I
        pr_info("[%s] No replica page found (I->I transition)\n", __func__);
    } else { // M -> S, S -> S
        pr_info("[%s] Page replica found (M->S or S->S transition)\n", __func__);
        ...
        if (ret == REPLICA_SUCCESS) {
            pr_info("[%s] M->S transition\n", __func__);
        } else if (ret == REPLICA_SHARED_STATE) {
            pr_info("[%s] S->S transition, no writeback needed\n", __func__);
        } else {
            pr_info("[%s] Failed to write-protect page replica: \n", __func__);
        }
...
static int swmc_kmsg_handle_invalidate(struct swmc_kmsg_message *msg)
{
    ...
    if (!page_replica) { // I -> I
        pr_info("[page_coherence] No page replica found (I->I transition)\n");
    }
    else { // M -> I, S -> I
        pr_info("[page_coherence] Page replica found (M->I or S->I transition)\n");
         ...
         * 
         * NOTE: page_replica_flush() internally handles all reference management
         * including the reference obtained from get_page_replica_with_ref(), so we should
         * NOT call put_page_replica_ref() afterwards to avoid double-free bugs.
         */
        put_page_replica_ref(page_replica);
        int invalidate_ret = destroy_page_replica(page_replica);
        if (invalidate_ret) {
            pr_info("[page_coherence] Failed to flush page replica: %d\n", invalidate_ret);
        } else {
            pr_info("[page_coherence] Successfully flushed page replica\n");
        }
    }
                         ...
                         size_t size, void *kaddr, pfn_t *pfn, pfn_t *pfnp)
{
        ...
        */
        ...
        if (page_replica->mapping) {
            ...
            if (check_page_replica_dirty(page_replica)) {
                ...
                pr_info("[%s] Replica page is dirty - M state\n", __func__);
            } else {
                /* Page is clean - likely in S state, but could be M with clean data */
                pr_info("[%s] Replica page is clean - S state\n", __func__);
    ...
    if (IS_ERR(page_replica)) {
        ...
        pr_info("[%s] Failed to create page replica: %d\n", __func__, ret);
        return ret;
    }
    ...
    pr_info("[%s] Created new page replica: %p\n", __func__, page_replica);

map_pfn:
    pr_info("[%s] Original page: %p, page replica: %p\n", __func__, original_page, page_replica);
