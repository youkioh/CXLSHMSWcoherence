...
static DEFINE_SPINLOCK(page_coherence_aspace_lock);
...
static const struct address_space_operations page_coherence_aops = {
	.releasepage = NULL,  /* We don't need special release handling */
...
static void __init init_page_coherence_aspace(void)
{
	...
	pr_info("[page_coherence] Initialized address space for replicas\n");
}
...
static int add_replica_to_cache(struct page *page, pgoff_t index)
{
	...
	pr_info("[page_coherence] Added replica page (pfn=%lu) to cache at index %lu\n",
...
/**
 * allocate_replica_pages - Allocate multiple pages for large replica with cache integration
 * @nr_pages: Number of pages to allocate
 * @pages: Array to store allocated pages
 * @base_index: Base index for cache insertion
 *
 * Returns: 0 on success, negative error code on failure
 */
static int allocate_replica_pages(unsigned int nr_pages, struct page **pages, pgoff_t base_index)
{
    unsigned int i;
    ...
    for (i = 0; i < nr_pages; i++) {
        ...
        if (!pages[i]) {
            ...
            /* Free already allocated pages and remove from cache */
            while (i > 0) {
                i--;
                if (pages[i]->mapping) {
        ...
        /* Add each page to cache for reclaim management */
        ret = add_replica_to_cache(pages[i], base_index + i);
        if (ret) {
            pr_err("[page_coherence] Failed to add page %u to cache: %d\n", i, ret);
            __free_page(pages[i]);
    ...
    pr_info("[page_coherence] Allocated %u reclaimable replica pages starting at index %lu\n", 
            nr_pages, base_index);
    return 0;
}
