	...
	struct fuse_file_info *fi)
{
	...
	(void) fi;
	inode = famfs_inode(req, ino);

	res = fstatat(inode->fd, "", &buf, AT_EMPTY_PATH | AT_SYMLINK_NOFOLLOW);
	if (res == -1)
		return (void) fuse_reply_err(req, errno);

	/*
	 * Keep lookup/getattr consistent for shadow-backed regular files.
	 * The backing object is a YAML metadata file, while logical file attrs
	 * (including size) come from parsed metadata.
	 */
	if (inode->fmeta && S_ISREG(buf.st_mode)) {
		buf.st_mode = inode->fmeta->fm_mode | 0100000;
		buf.st_uid = inode->fmeta->fm_uid;
		buf.st_gid = inode->fmeta->fm_gid;
		buf.st_size = inode->fmeta->fm_size;
