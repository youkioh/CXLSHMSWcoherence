...
static void gic_send_sgi(u64 cluster_id, u16 tlist, unsigned int irq)
{
	...
	//printk(pr_fmt("CPU%d: ICC_SGI1R_EL1 %llx\n"), smp_processor_id(), val);
	gic_write_sgi1r(val);
}

static void gic_raise_softirq(const struct cpumask *mask, unsigned int irq)
{
	// printk(pr_fmt("calling gic_raise_softirq\n"));
	int cpu;
	...
	// printk(pr_fmt("cpu number in total are %d\n"), nr_cpu_ids);

	for ((cpu) = -1; (cpu) = cpumask_next((cpu), (mask)), (cpu) <= nr_cpu_ids;) {
		// printk(pr_fmt("current cpu id is %d\n"), cpu);
	// for_each_cpu(cpu, mask) {
		u64 cluster_id = MPIDR_TO_SGI_CLUSTER_ID(cpu_logical_map(cpu));
		u16 tlist;

		tlist = gic_compute_target_list(&cpu, mask, cluster_id);
		//printk(pr_fmt("gic raise soft irq: target list is %x\n"), tlist);
