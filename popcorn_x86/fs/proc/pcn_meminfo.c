...
#else
	rem_mem = send_remote_mem_info_request(1);
#endif
        if(rem_mem){
...
static int __init proc_popcorn_meminfo_init(void)
{
	proc_create_single("popcorn_meminfo", 0, NULL, meminfo_proc_show);
	return 0;
}
fs_initcall(proc_popcorn_meminfo_init);
