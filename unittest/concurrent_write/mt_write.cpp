...
void increment_thread(SharedData* shared_data, int process_id, int thread_id, int iterations) {
...
void worker_process(const char* filename, int process_id, int threads_per_process, int iterations_per_thread) {
    ...
    // 프로세스 준비 완료 신호
    shared_data->ready_processes.fetch_add(1);
    
    // 모든 프로세스가 준비될 때까지 대기
    while (!shared_data->start_flag.load()) {
    ...
    for (int i = 0; i < threads_per_process; i++) {
        threads.emplace_back(increment_thread, shared_data, process_id, i, iterations_per_thread);
    }
...
int main(int argc, char* argv[]) {
    ...
    shared_data->ready_processes.store(0);
    shared_data->start_flag.store(false);
    ...
    while (shared_data->ready_processes.load() < num_processes) {
        std::this_thread::sleep_for(std::chrono::milliseconds(1));
    }
    
    std::cout << "All processes ready! Starting test..." << std::endl;
    shared_data->start_flag.store(true);
